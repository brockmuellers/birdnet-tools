...
- Refreshes the species frequency cache weekly (`scripts/refresh_species_freq.py`). Queries the BirdNET-Pi metadata model for all ~6,500 species at the current week and location; writes `.species_freq_cache.json` with a `species_frequencies` map for species in the band `0 < frequency <= SF_THRESH` (read from `/etc/birdnet/birdnet.conf`). Used by `push_events.py` to surface exclusion events only for species that are present in the region but below the threshold.
- Aggregates events and pushes them to Cloudflare R2 every 15 minutes (`scripts/push_events.py`). Sources:
	- `birdnet_analysis` systemd journal: species frequency exclusions for species in the `(0, SF_THRESH]` band (INFO) and errors (ERROR); exclusions for zero-frequency species (not expected in this region) are suppressed using `.species_freq_cache.json`
	...
	- `metric_samples.jsonl`: periodic numeric samples from `scripts/sample_metrics.py` (every 5 min): `temp_c`, `memory_available_mb`, `wifi_signal_dbm`, `disk_root_used_pct`, `disk_root_free_gb`, `disk_backup_used_pct`, `disk_backup_free_gb`, `disk_io_read_mb`, `disk_io_write_mb` (MB read/written on the root device since the previous sample; absent on first run)
